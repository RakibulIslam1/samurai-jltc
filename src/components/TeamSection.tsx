...
function roleLabel(member: TeamMember) {
  if (member.role === 'chairman') return 'Chairman'
  if (member.role === 'managing-director') return 'Managing Director'
  if (member.role === 'custom') return member.customRole || 'Member'
...
export default function TeamSection() {
  ...
  const customMembers = useMemo(() => members.filter((member) => member.role === 'custom'), [members])
  ...
  return (
    <section className="bg-gray-50 py-20" aria-labelledby="team-heading">
      <div className="mx-auto max-w-6xl px-4 text-center sm:px-6 lg:px-8">
        ...
        {!loading && !chairman && !managingDirector && instructors.length === 0 && customMembers.length === 0 && (
            ...
            {leadership.map((member) => (
              <article key={member.id} className="w-full max-w-[260px] overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
                ...
                <div className="p-4 text-center">
                  <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-primary">{roleLabel(member)}</p>
            ...
            <div className="grid grid-cols-1 gap-6 justify-items-center sm:grid-cols-2 lg:grid-cols-3">
              {instructors.map((member) => (
                <article key={member.id} className="w-full max-w-[260px] overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
                  ...
                  <div className="p-4 text-center">
                    <p className="mb-1 text-xs font-semibold uppercase tracking-wide text-primary">{roleLabel(member)}</p>
        ...
        {!loading && customMembers.length > 0 && (
          <div className={`grid grid-cols-1 gap-6 justify-items-center sm:grid-cols-2 lg:grid-cols-3 ${instructors.length > 0 ? 'mt-8' : ''}`}>
